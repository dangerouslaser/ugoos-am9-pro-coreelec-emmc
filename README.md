...
1. Verifies you're on a supported board (reading `coreelec-dt-id` from the active `/flash/dtb.img` and matching it against `SUPPORTED_BOARDS` — see [the technical note](#why-the-board-check-reads-coreelec-dt-id-instead-of-hashing-the-dtb)) and booting from removable media, and that the eMMC still has the expected Android layout — partition **names** (`super`/`rsv`/`userdata`) are checked, not just numbers, so a partially-completed previous run is caught here instead of poisoning the backups below
