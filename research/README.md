...
| [`installer-design-notes.md`](installer-design-notes.md) | Why the eMMC installer is built the way it is: CE_FLASH at p28, the cfgload rebuild and why `mount-storage.sh` + `nofsck` are the real survival layer, the `coreelec-dt-id` board check, the device-number eMMC-mount check, and what tooling CoreELEC lacks. |
